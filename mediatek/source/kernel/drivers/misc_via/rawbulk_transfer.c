...
static void upstream_second_stage(struct urb *urb) {
    ...
    printk(KERN_DEBUG "t %p start upstream\n", t);
    if (urb->status < 0) {
    ...
    printk(KERN_DEBUG "t %p pass to musb\n", t);
...
static void upstream_final_stage(struct usb_ep *ep,
    ...
    printk(KERN_DEBUG "t %p musb finished\n", t);
    if (req->status < 0) {
...
static void downstream_second_stage(struct usb_ep *ep,
    ...
    printk(KERN_DEBUG "t %p start downstream\n", t);
    if (req->status) {
        if (req->status != -ECONNRESET)
            terr(t, "req status %d", req->status);
        if (req->status == -ESHUTDOWN)
    ...
    */
    ...
    do {
        ...
        printk(KERN_DEBUG "t %p pass to ohci\n", t);
...
static void downstream_final_stage(struct urb *urb) {
    ...
    printk(KERN_DEBUG "t %p urb finished\n", t);
