...
static struct _function_init_stuff {
    ...
    const char *iString;
    unsigned int out_maxpkgsz;
    unsigned int in_maxpkgsz;
    unsigned int nups;
    unsigned int ndowns;
    unsigned int upsz;
    unsigned int downsz;
    ...
    {"rawbulk-modem", "data", "Modem Port", 64, 64, 16, 16, PAGE_SIZE, PAGE_SIZE, PAGE_SIZE, true, false },
    {"rawbulk-ets", "ets", "ETS Port", 64, 64, 4, 4, PAGE_SIZE, PAGE_SIZE, PAGE_SIZE, true, false },
    {"rawbulk-at", "atc", "AT Channel", 64, 64, 1, 1, PAGE_SIZE, PAGE_SIZE, PAGE_SIZE, true, false },
    {"rawbulk-pcv", "pcv", "PCM Voice", 64, 64, 1, 1, PAGE_SIZE, PAGE_SIZE, PAGE_SIZE, true, false },
    {"rawbulk-gps", "gps", "LBS GPS Port", 64, 64, 1, 1, PAGE_SIZE, PAGE_SIZE, PAGE_SIZE, true, false },
...
static __init struct rawbulk_function *rawbulk_alloc_function(int transfer_id) {
    int rc;
    struct rawbulk_function *fn;
    struct _function_init_stuff *_init;
    ...
    _init = &_init_params[transfer_id];
    ...
    init_endpoint_desc(&fn->hs_bulkin_endpoint, 1, _init->in_maxpkgsz);
    init_endpoint_desc(&fn->hs_bulkout_endpoint, 0, _init->out_maxpkgsz);
    ...
    add_device_attr(fn, ATTR_STATISTICS, "statistics", 0440);
    ...
    add_device_attr(fn, ATTR_SPLIT, "split_size", 0660);
    add_device_attr(fn, ATTR_PUSHABLE, "pushable", 0660);
