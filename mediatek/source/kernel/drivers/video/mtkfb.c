...
static u32 MTK_FB_XRES  = 0;//DISP_GetScreenWidth();540
static u32 MTK_FB_YRES  = 0;//DISP_GetScreenHeight();960
static u32 MTK_FB_BPP   = 0;//32
static u32 MTK_FB_PAGES = 0;// 2
static u32 fb_xres_update = 0;//MTK_FB_XRES 540
static u32 fb_yres_update = 0;//MTK_FB_YRES 960
#define ALIGN_TO(x, n)  \
	(((x) + ((n) - 1)) & ~((n) - 1))
...
#define MTK_FB_LINE  (ALIGN_TO(MTK_FB_XRES, 32) * MTK_FB_BYPP)//544*4=2176
#define MTK_FB_SIZE  (MTK_FB_LINE * ALIGN_TO(MTK_FB_YRES, 32))

#define MTK_FB_SIZEV (MTK_FB_LINE * ALIGN_TO(MTK_FB_YRES, 32) * MTK_FB_PAGES)//2176*960*2
...
#if defined(TINNO_ANDROID_S9091) && !defined(BUILD_UBOOT) && !defined(BUILD_LK)
{
	...
	if ( !flush_flag ){
	       printk("[FB Driver] set flush_flag = 1\n");
		flush_flag = 1;
...
//Tinno:CJ
#ifdef TINNO_FILL_BLACK_FB
#undef TINNO_FILL_BLACK_FB
#endif
#ifdef TINNO_FILL_BLACK_FB
//Tinno:CJ
struct fb_info		   *g_fbi = 0;
#endif
...
#ifdef TINNO_FILL_BLACK_FB
static void set_black_screen()
{
    // clean whole frame buffer as black
    if(g_fbi != 0)
   	{
   	       void *buffer = g_fbi->screen_base + g_fbi->var.yoffset * g_fbi->fix.line_length;
    		u32 bpp = (g_fbi->var.bits_per_pixel + 7) >> 3;
              u32 size = g_fbi->var.xres * g_fbi->var.yres * bpp;

	       //unsigned int mva;
	       //void *buffer = kmalloc(size , GFP_KERNEL);
	       //memset(buffer, 1, size);
		//LCD_AllocUIMva(buffer, &mva, DAL_GetLayerSize())
		LCD_CHECK_RET(LCD_LayerEnable(FB_LAYER - 1, TRUE));
		LCD_CHECK_RET(LCD_LayerSetAddress(FB_LAYER - 1, (UINT32)buffer));
		LCD_CHECK_RET(LCD_LayerSetFormat(FB_LAYER - 1, LCD_LAYER_FORMAT_RGB565));
		LCD_CHECK_RET(LCD_LayerSetOffset(FB_LAYER - 1, 0, 0));
		LCD_CHECK_RET(LCD_LayerSetSize(FB_LAYER - 1, MTK_FB_XRES, MTK_FB_YRES));
		LCD_CHECK_RET(LCD_LayerSetPitch(FB_LAYER - 1, ALIGN_TO(MTK_FB_YRES, 32)*2));

		mtkfb_update_screen(g_fbi);

		printk("[FB Driver] set_black_screen\n");
	}
}
#endif
