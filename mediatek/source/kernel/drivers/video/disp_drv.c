...
static unsigned char lcm_init_settings[LCM_INIT_TABLE_SIZE_MAX] = {REGFLAG_END_OF_TABLE};
static ssize_t lcm_settings_read(struct file *file, char __user *ubuf, size_t count, loff_t *offset)
{
	int rc = 0;
	if(lcm_drv && lcm_drv->get_initialization_settings){
		memset(lcm_init_settings, REGFLAG_END_OF_TABLE, LCM_INIT_TABLE_SIZE_MAX);
		if (down_interruptible(&sem_update_screen)) {
			printk(KERN_ERR "Can't get sem_update_screen in lcm_settings_read()\n");
			rc = -EAGAIN;
		}
		rc = lcm_drv->get_initialization_settings(lcm_init_settings);
		up(&sem_update_screen);
		if ( rc > 0 ){
			rc = simple_read_from_buffer(ubuf, count, offset,  
										lcm_init_settings, rc);
		}
	}else{
		rc = -EACCES;
	}
...
static ssize_t lcm_settings_write(struct file *file, const char __user *buf, size_t count, loff_t *offset)
{
	int rc = 0;
	char __user *start = buf;
	printk(KERN_ERR "lcm_settings_write: count = %d, offset=%d", count, (int)(*offset));
	
	if ( count > LCM_INIT_TABLE_SIZE_MAX || 0 == count ){
		printk(KERN_ERR "LCM settings code is too long.");
		return -EDOM;
	}
	
	if ( 0 == count ){
		printk(KERN_ERR "LCM settings code is zore.");
		return -EDOM;
	}

	if ( copy_from_user(lcm_init_settings, start, count) ){
		printk(KERN_ERR "lcm_settings_write: copy failed(%d)", rc);
		return -EACCES;
	}

	if(lcm_drv && lcm_drv->set_initialization_settings){
		...
		if ( down_interruptible(&sem_flipping) ){
			printk(KERN_ERR "Can't get sem_flipping in lcm_settings_write()\n");
			return -EAGAIN;
		}
		if ( down_interruptible(&sem_early_suspend) ){
			up(&sem_flipping);
			printk(KERN_ERR "Can't get sem_early_suspend in lcm_settings_write()\n");
			return -EAGAIN;
		}
		...
		if (down_interruptible(&sem_update_screen)) {
			up(&sem_early_suspend);
			up(&sem_flipping);
			printk(KERN_ERR "Can't get sem_update_screen in lcm_settings_write()\n");
			return -EAGAIN;
		}
		...
		rc = lcm_drv->set_initialization_settings(lcm_init_settings, count);
		up(&sem_update_screen);
		up(&sem_early_suspend);
		up(&sem_flipping);
		if (rc < count) {
			printk(KERN_ERR "lcm_settings_write:  failed(%d)", rc);
			rc = -EIO;
		} 
	}else{
		rc = -EACCES;
...
static const struct file_operations lcm_settings_fops = {
	.owner = THIS_MODULE,
	.read = lcm_settings_read,
	.write = lcm_settings_write,
};

static struct miscdevice lcm_settings_device = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = "lcm_settings",
	.fops = &lcm_settings_fops,
};
...
#ifndef BUILD_UBOOT    
	if ( DISP_STATUS_OK == r ){//Create a misc dev to show the name of LCM.
		...
		ret = misc_register(&lcm_settings_device);
		if (ret) {
			misc_deregister(&lcm_settings_device);
			printk(KERN_ERR "lcm_settings_device device register failed (%d)\n", ret);
		}
	}
#endif
...
#if defined(TINNO_ANDROID_S9091) && !defined(BUILD_UBOOT) && !defined(BUILD_LK)
{
		extern volatile int flush_flag;
		printk("[FB Driver] set flush_flag = 0\n");
		flush_flag = 0;
}
#endif
		lcm_drv->suspend();
