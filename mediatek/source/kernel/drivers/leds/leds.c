...
volatile int flush_flag = 1;
DECLARE_WAIT_QUEUE_HEAD(lcm_flush_wait);
#endif
...
static void mt65xx_led_set(struct led_classdev *led_cdev, enum led_brightness level)
{
	...
	if (led_data->level != level) {
		...
		}else
		{
				...
				if(level && !flush_flag) {
					int rc;
					printk("%s: wait LCM flush(flush_flag=%d), then turn on BL later--Jieve\n", 
						__func__, flush_flag);
					rc = wait_event_interruptible_timeout(lcm_flush_wait, flush_flag, HZ/10);
					if ( 0 == rc ){
						printk("%s: wait LCM flush time out--Jieve\n", __func__);
					} 
				}
			#endif
				mt65xx_led_set_cust(&led_data->cust, led_data->level);	
