...
int al3006_init_device(struct i2c_client *client)
{
    ...
    //LINE<JIRA ID> <DATE20120613> <window loss = 5> zenghaihui
    ...
    //LINE<JIRA ID> <DATE20120711> <ps accuracy count=6,&ps threshold =5> zenghaihui
    //if(hwmsen_write_byte(client,APS_PS_CTL,0x4A))//ps accuracy count=7 &ps threshold =A
    if(hwmsen_write_byte(client,APS_PS_CTL,0x05))//ps accuracy count=6 &ps threshold =5
    {
      return -EFAULT;
    }
...
static void al3006_eint_work(struct work_struct *work)
{
	...
	#endif
//LINE<JIRA ID> <DATE20120606> <ÖÐ¶ÏÎÊÌâ£¬MP °æ±Ÿ¹ÀŒÆ»ážüžÄ¹ýÁË£¬ºóÐø×¢Òâ> zenghaihui
#if 0 // def MT6577
	mt65xx_eint_unmask(CUST_EINT_ALS_NUM);  
#endif
}
...
static int al3006_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    //LINE<JIRA ID> <DATE20120611> <BUG INFO> zenghaihui
