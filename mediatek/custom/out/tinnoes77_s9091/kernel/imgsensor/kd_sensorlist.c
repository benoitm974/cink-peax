...
static char g_currSensorNames[2][32] = {};
static char *sensorname_main =& g_currSensorNames[1][0];
static char *sensorname_sub = &g_currSensorNames[0][0];
...
inline static int adopt_CAMERA_HW_CheckIsAlive(void)
{
	...
	else
	{
	     ...
	     if(sensorinum++)
	     {
		strcpy(&g_currSensorNames[0][0],g_currSensorName);
	     }
	     else
	     {
	       if((sensorID==OV8825_SENSOR_ID)||(sensorID==OV8826_SENSOR_ID))
	       {
			extern volatile int  camera_sensor_type;

			if(camera_sensor_type==1)
			{
				strcpy(&g_currSensorNames[1][0],"ov8825-truly");
			}
			else
			{
				strcpy(&g_currSensorNames[1][0],"ov8825-shunyu");
			}
		}
		else
		{
			strcpy(&g_currSensorNames[1][0],g_currSensorName);
		}
	     }
        }
    }
...
//            }
	...
	switch (pFeatureCtrl->FeatureId)
	{
        ...
        case SENSOR_FEATURE_CHECK_ALIVE:
            //
