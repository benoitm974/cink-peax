...
int i=0;
for(i=0;i<sizeof(kdFlashlightList)/sizeof(KD_FLASHLIGHT_INIT_FUNCTION_STRUCT);i++)
{
   if(kdFlashlightList[i].flashlightId==flashlightIdx)
   	break;
}
if(i==sizeof(kdFlashlightList)/sizeof(KD_FLASHLIGHT_INIT_FUNCTION_STRUCT))
{
printk("not find flash ID :%d" ,flashlightIdx);
  return 0;
}
	
    if (NULL != kdFlashlightList[i].flashlightInit) {
        kdFlashlightList[i].flashlightInit(&g_pFlashlightFunc);
