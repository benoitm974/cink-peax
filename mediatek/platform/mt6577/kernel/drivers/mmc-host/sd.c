...
#ifdef TINNO_ANDROID_MSDC_REMOVEABLE
static int g_card_insert= 0;
#endif
...
{     
	   ...
	   if(on)
	   	  ...
	   	  hwPowerDown_fpga();
#else
#if defined(TINNO_ANDROID_S9050_SDIO) || defined(TINNO_ANDROID_S9091_SDIO)
	mt6575_mmc_info("%s: on=%d--Liu\n", __func__, on);
	if(on)
		hwPowerOn(MT65XX_POWER_LDO_VGP2, VOL_3300, "msdc");
	else
		hwPowerDown(MT65XX_POWER_LDO_VGP2, "msdc");
#else
    ...
    if(on)
    {
	msdc_ldo_power(1, MT65XX_POWER_LDO_VMCH, powerVolt, &g_vmch);
    }
    else if(g_card_insert)	
    {
	msdc_ldo_power(0, MT65XX_POWER_LDO_VMCH, powerVolt, &g_vmch);
    }
#else
...
#include "../../../../../../kernel/drivers/mmc/core/sd_ops.h"
#include "../../../../../../kernel/drivers/mmc/core/mmc_ops.h"
static char *state[] = {
	"Idle", 		/* 0 */
	"Ready",		/* 1 */
	"Ident",		/* 2 */
...
int msdc_decode_card_resp(uint32_t card_status) 
{
	static char *state[] = {
		"Idle", 		/* 0 */
		"Ready",		/* 1 */
	...
	if (card_status & R1_OUT_OF_RANGE)
		mt6575_mmc_info("\t[CARD_STATUS] Out of Range\n");
	if (card_status & R1_ADDRESS_ERROR)
		mt6575_mmc_info("\t[CARD_STATUS] Address Error\n");
	if (card_status & R1_BLOCK_LEN_ERROR)
		mt6575_mmc_info("\t[CARD_STATUS] Block Len Error\n");
	if (card_status & R1_ERASE_SEQ_ERROR)
		mt6575_mmc_info("\t[CARD_STATUS] Erase Seq Error\n");
	if (card_status & R1_ERASE_PARAM)
		mt6575_mmc_info("\t[CARD_STATUS] Erase Param\n");
	if (card_status & R1_WP_VIOLATION)
		mt6575_mmc_info("\t[CARD_STATUS] WP Violation\n");
	if (card_status & R1_CARD_IS_LOCKED)
		mt6575_mmc_info("\t[CARD_STATUS] Card is Locked\n");
	if (card_status & R1_LOCK_UNLOCK_FAILED)
		mt6575_mmc_info("\t[CARD_STATUS] Lock/Unlock Failed\n");
	if (card_status & R1_COM_CRC_ERROR)
		mt6575_mmc_info("\t[CARD_STATUS] Command CRC Error\n");
	if (card_status & R1_ILLEGAL_COMMAND)
		mt6575_mmc_info("\t[CARD_STATUS] Illegal Command\n");
	if (card_status & R1_CARD_ECC_FAILED)
		mt6575_mmc_info("\t[CARD_STATUS] Card ECC Failed\n");
	if (card_status & R1_CC_ERROR)
		mt6575_mmc_info("\t[CARD_STATUS] CC Error\n");
	if (card_status & R1_ERROR)
		mt6575_mmc_info("\t[CARD_STATUS] Error\n");
	if (card_status & R1_UNDERRUN)
		mt6575_mmc_info("\t[CARD_STATUS] Underrun\n");
	if (card_status & R1_OVERRUN)
		mt6575_mmc_info("\t[CARD_STATUS] Overrun\n");
	if (card_status & R1_CID_CSD_OVERWRITE)
		mt6575_mmc_info("\t[CARD_STATUS] CID/CSD Overwrite\n");
	if (card_status & R1_WP_ERASE_SKIP)
		mt6575_mmc_info("\t[CARD_STATUS] WP Eraser Skip\n");
	if (card_status & R1_CARD_ECC_DISABLED)
		mt6575_mmc_info("\t[CARD_STATUS] Card ECC Disabled\n");
	if (card_status & R1_ERASE_RESET)
		mt6575_mmc_info("\t[CARD_STATUS] Erase Reset\n");
	if (card_status & R1_READY_FOR_DATA)
		mt6575_mmc_info("\t[CARD_STATUS] Ready for Data\n");
	if (card_status & R1_SWITCH_ERROR)
		mt6575_mmc_info("\t[CARD_STATUS] Switch error\n");
	//if (card_status & R1_URGENT_BKOPS)
		//mt6575_mmc_info("\t[CARD_STATUS] Urgent background operations\n");
	if (card_status & R1_APP_CMD)
		mt6575_mmc_info("\t[CARD_STATUS] App Command\n");

	mt6575_mmc_info("\t[CARD_STATUS] '%s' State\n", state[R1_CURRENT_STATE(card_status)]);
} 
int msdc_lock_unlock_test(int mode)
{
	struct mmc_host *host = msdc_6577_host[1]->mmc;
	char buf[] = {0x01, 0x02, 0x03, 0x04};
	int rc = 0;
	if(host) {
		if(host->card) {
			rc = mmc_sd_lock_unlock(host->card, mode, buf, sizeof(buf));
		}
		else {
			printk("%s: card is null--Liu\n", __func__);
			rc = -1;
		}
	}
	return rc;
}
EXPORT_SYMBOL(msdc_lock_unlock_test);

void force_rescan_mmc(int slot, int msec)
{
	struct mmc_host *mmc = msdc_6577_host[slot]->mmc;
	if(mmc) {
		mt6575_mmc_info("%s: slot=%d--Liu\n", __func__, slot);
		mmc_detect_change(mmc, msecs_to_jiffies(msec));
	}
	else {
		mt6575_mmc_info("%s: error, slot=%d, not registered--Liu\n", __func__, slot);
	}
}
EXPORT_SYMBOL(force_rescan_mmc);

extern void mmc_host_remove_card(struct mmc_host *host);

static ssize_t sd_secure_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	int sd_security = 0;
	...
	if(mmc) {
		if(mmc->card) {
			sd_security = mmc->card->sec_info.sd_security;
		}
	}
	mt6575_mmc_info("%s: sd_security=%x--Liu\n", __func__, sd_security);
	return sprintf(buf, "%d\n", sd_security);
}
static ssize_t sd_secure_store(
...
static DEVICE_ATTR(sd_secure, S_IRUGO | S_IWUSR, sd_secure_show, sd_secure_store);

static ssize_t sd_lock_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	int sd_card_locked = 0;
	struct mmc_host *mmc = dev_get_drvdata(dev);
	if(mmc)
		sd_card_locked = mmc->sdcard_locked;
	mt6575_mmc_info("%s: sd_card_locked=%x--Liu\n", __func__, mmc->sdcard_locked);
	return sprintf(buf, "%d\n", mmc->sdcard_locked);

}
static ssize_t sd_lock_store(
		struct device *dev, struct device_attribute *attr,
		const char *buf, size_t size)
{
	mt6575_mmc_info("%s--Liu\n", __func__);
...
static DEVICE_ATTR(sd_lock, S_IRUGO | S_IWUSR, sd_lock_show, sd_lock_store);

static ssize_t sd_unlock_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	int sd_unlocked = 1;
	struct mmc_host *mmc = dev_get_drvdata(dev);
	if(mmc)
		sd_unlocked = !mmc->sdcard_locked;
	mt6575_mmc_info("%s: sd_unlocked=%x--Liu\n", __func__, sd_unlocked);
	return sprintf(buf, "%d\n", sd_unlocked);
}
static ssize_t sd_unlock_store(
		struct device *dev, struct device_attribute *attr,
		const char *buf, size_t size)
{
	int retries = 10;
	...
	if(mmc) {
		if(buf[size - 1] == '\n') 
			--size;
		if(!size) {
			mt6575_mmc_info("%s: size is 0--Liu\n", 
				__func__, buf, size);
			return size;
		}
		//Remove card first
		//mmc_host_remove_card(mmc);
		if(mmc->passwd)
			kfree(mmc->passwd);
		mmc->passwd = (unsigned char *)kzalloc(size, GFP_KERNEL);
		memcpy((void *)mmc->passwd, (void *)buf, size);
		mmc->lock_unlock_mode = 0;
		mmc->passwd_length = size;
		mt6575_mmc_info("%s: buf=%s, size=%d--Liu\n", 
			__func__, mmc->passwd, mmc->passwd_length);
		while(retries) {
			msleep(100);
			force_rescan_mmc(1, 0);
			if(!mmc->sdcard_locked) {
				mt6575_mmc_info("%s: retries=%d, unlock success--Liu\n", 
					__func__, retries);
				break;
			}
			else {
		...
		if(!retries) {
			mt6575_mmc_info("%s: too many retries--Liu\n", __func__);
		}
	
	}
	return old_size;
}
static DEVICE_ATTR(sd_unlock, S_IRUGO | S_IWUSR, sd_unlock_show, sd_unlock_store);

static ssize_t sd_passwd_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	int value = 1;
	struct mmc_host *mmc = dev_get_drvdata(dev);
	mt6575_mmc_info("%s: passwd_length=%d, bsec=%d--Liu\n", 
		__func__, mmc->passwd_length, mmc->bsec);
	if(!mmc->bsec || !mmc->passwd_length)
		value = 0;
	return sprintf(buf, "%d\n", value); 
}
static ssize_t sd_passwd_store(
		struct device *dev, struct device_attribute *attr,
		const char *buf, size_t size)
{
	struct mmc_host *mmc = dev_get_drvdata(dev);
...
static DEVICE_ATTR(sd_passwd, S_IRUGO | S_IWUSR, sd_passwd_show, sd_passwd_store);

static ssize_t sd_clear_passwd_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	mt6575_mmc_info("%s--Liu\n", __func__);
	return sprintf(buf, "secret\n"); 
}
static ssize_t sd_clear_passwd_store(
		struct device *dev, struct device_attribute *attr,
		const char *buf, size_t size)
{
	int retries = 10;
	...
	if(mmc) {
		...
		while(retries) {
			...
			force_rescan_mmc(1, 0);
			if(!mmc->sdcard_locked) {
				mt6575_mmc_info("%s: retries=%d, clear success--Liu\n", 
					__func__, retries);
				break;
			}
			else {
...
static DEVICE_ATTR(sd_clear_passwd, S_IRUGO | S_IWUSR, sd_clear_passwd_show, sd_clear_passwd_store);

static ssize_t sd_erase_all_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	mt6575_mmc_info("%s--Liu\n", __func__);
	return sprintf(buf, "secret\n"); 
}
static ssize_t sd_erase_all_store(
		struct device *dev, struct device_attribute *attr,
		const char *buf, size_t size)
{
	struct mmc_host *mmc = dev_get_drvdata(dev);
...
static DEVICE_ATTR(sd_erase_all, S_IRUGO | S_IWUSR, sd_erase_all_show, sd_erase_all_store);

static ssize_t sd_passwd_error_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	int passwd_error = 0;
	struct mmc_host *mmc = dev_get_drvdata(dev);
	if(mmc)
		passwd_error = mmc->passwd_error;
	mt6575_mmc_info("%s: passwd_error=%d--Liu\n", __func__, passwd_error);
	return sprintf(buf, "%d\n", passwd_error);

}
static ssize_t sd_passwd_error_store(
		struct device *dev, struct device_attribute *attr,
		const char *buf, size_t size)
{
	mt6575_mmc_info("%s--Liu\n", __func__);
...
static DEVICE_ATTR(sd_passwd_error, S_IRUGO | S_IWUSR, sd_passwd_error_show, sd_passwd_error_store);

static ssize_t sd_in_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	int sdcard_in = 0;
	struct mmc_host *mmc = dev_get_drvdata(dev);
	if(mmc)
		sdcard_in = mmc->sdcard_in;
	mt6575_mmc_info("%s: sdcard_in=%d--Liu\n", __func__, sdcard_in);
	return sprintf(buf, "%d\n", sdcard_in);

}
static ssize_t sd_in_store(
		struct device *dev, struct device_attribute *attr,
		const char *buf, size_t size)
{
	mt6575_mmc_info("%s--Liu\n", __func__);
...
static DEVICE_ATTR(sd_in, S_IRUGO | S_IWUSR, sd_in_show, sd_in_store);
#endif
                                      ...
                                      unsigned long       timeout)
{
    ...
    if (opcode == MMC_SEND_OP_COND || opcode == SD_APP_OP_COND)
        ...
        resp = RESP_R1;
#ifdef TINNO_ANDROID_SD_SECURE
	else if (opcode == MMC_LOCK_UNLOCK) {
		resp = RESP_R1B;
	}
#endif
    else {
                                      ...
                                      unsigned long       timeout)
{
    ...
    host->cmd = NULL;
#ifdef TINNO_ANDROID_SD_SECURE
	if((opcode == MMC_SELECT_CARD) || (opcode == MMC_LOCK_UNLOCK)) {
		msdc_decode_card_resp(cmd->resp[0]);
	}
#endif
    /* if (resp == RESP_R1B) {
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	//only for sdcard
	if(pdev->id == 1) {
		ret = device_create_file(&pdev->dev, &dev_attr_sd_secure);
		if(ret) {
			mt6575_mmc_info("%s: create sd_secure file error=%d!--Liu\n", 
				__func__, ret);
		}
		ret = device_create_file(&pdev->dev, &dev_attr_sd_lock);
		if(ret) {
			mt6575_mmc_info("%s: create sd_lock file error=%d!--Liu\n", 
				__func__, ret);
		}
		ret = device_create_file(&pdev->dev, &dev_attr_sd_unlock);
		if(ret) {
			mt6575_mmc_info("%s: create sd_unlock file error=%d!--Liu\n", 
				__func__, ret);
		}
		ret = device_create_file(&pdev->dev, &dev_attr_sd_passwd);
		if(ret) {
			mt6575_mmc_info("%s: create sd_passwd file error=%d!--Liu\n", 
				__func__, ret);
		}
		ret = device_create_file(&pdev->dev, &dev_attr_sd_clear_passwd);
		if(ret) {
			mt6575_mmc_info("%s: create sd_clear_passwd file error=%d!--Liu\n", 
				__func__, ret);
		}
		ret = device_create_file(&pdev->dev, &dev_attr_sd_erase_all);
		if(ret) {
			mt6575_mmc_info("%s: create sd_erase_all file error=%d!--Liu\n", 
				__func__, ret);
		}
		ret = device_create_file(&pdev->dev, &dev_attr_sd_passwd_error);
		if(ret) {
			mt6575_mmc_info("%s: create sd_passwd_error file error=%d!--Liu\n", 
				__func__, ret);
		}
		ret = device_create_file(&pdev->dev, &dev_attr_sd_in);
		if(ret) {
			mt6575_mmc_info("%s: create sd_in file error=%d!--Liu\n", 
				__func__, ret);
		}
	}
#endif
