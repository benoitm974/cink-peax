...
#define    DL1_USE_SYSRAM //Audio DL1 playback use slave mode by MMsys memory
    ...
    #if defined(EVDO_DT_SUPPORT)&&defined(GPIO_VIA_REC_SW)
    #define    DT_RECEIVER_SWITCH_GPIO GPIO_VIA_REC_SW
    #else
    #define    DT_RECEIVER_SWITCH_GPIO  GPIO_52_REC_SW
    #endif
#endif
/*****************************************************************************
*                         D A T A      T Y P E S
...
// for MT519x(ATV) & MT6620(FM) use I2S path for the same project
      ...
      case AUDDRV_SET_RECEIVER_GPIO:
      {
          ...
          if(arg == 0)
          {
              mt_set_gpio_dir(DT_RECEIVER_SWITCH_GPIO, GPIO_DIR_OUT); //output
              mt_set_gpio_out(DT_RECEIVER_SWITCH_GPIO, GPIO_OUT_ZERO);//low mt6573
          }
          else if (arg == 1)
          {
              mt_set_gpio_dir(DT_RECEIVER_SWITCH_GPIO, GPIO_DIR_OUT); //output
              mt_set_gpio_out(DT_RECEIVER_SWITCH_GPIO, GPIO_OUT_ONE);//high mt6252
          }
