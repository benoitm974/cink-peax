...
{   
    ...
    if (status.CMD_DONE)
    {
    	...
    	// Go back to LP mode.
    	if(!DSI_REG->DSI_STA.BUSY){//Jieve Liu
		DSI_clk_HS_mode(0); 
	}
...
extern LCM_PARAMS *lcm_params;//Jieve Liu
...
void DSI_set_cmdq_V2(unsigned cmd, unsigned char count, unsigned char *para_list, unsigned char force_update)
{
	...
	if (count > 59)
	{
		...
		t1.CONFG = 1;
		if (cmd < 0xB0 ||  lcm_params->dsi.force_dcs_packet)//Jieve Liu
