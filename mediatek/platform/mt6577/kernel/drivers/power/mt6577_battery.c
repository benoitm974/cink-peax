...
#define DEBUG_TEMPERATURE_PROTECT

#ifdef DEBUG_TEMPERATURE_PROTECT
int use_fake_temp_flag=0;
int fake_temp=0;
#endif
...
printk("table_size=%d",table_size);
...
void select_charging_curret(void)
{
	...
	{    
		...
		{   
			g_temp_CC_value = AC_CHARGER_CURRENT ;//USB_CHARGER_CURRENT;
...
void mt_battery_notify_check(void)
{
	...
	if(g_BN_TestMode == 0x0000)
	{
		...
		if( (BMT_status.ICharging > 1350) && //9091 ±êÅä1000mAµÄ³äµçÆ÷.
...
#ifdef DEBUG_TEMPERATURE_PROTECT
static ssize_t show_fake_temp_value(struct device *dev,struct device_attribute *attr, char *buf)
{
	int ret_value=1;
	ret_value = fake_temp;
	if(use_fake_temp_flag==0)
	{
	  return sprintf(buf, "fake temp is not valid.\n", ret_value);
	}else{
	return sprintf(buf, "fake temp :%d\n", ret_value);
		}
}

static ssize_t store_fake_temp_value(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[EM] Not Support Write Function\n");	
	if(buf!=NULL)
	{
          sscanf(buf ,"%d %d",&use_fake_temp_flag,&fake_temp);
          printk("use_fake_temp_flag=%d, fake_temp=%d \n",use_fake_temp_flag,fake_temp);
	}
	return size;
}
static DEVICE_ATTR(Battery_Fake_Temp_Set, 0664, show_fake_temp_value, store_fake_temp_value);
...
#ifdef DEBUG_TEMPERATURE_PROTECT
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Battery_Fake_Temp_Set);
#endif
