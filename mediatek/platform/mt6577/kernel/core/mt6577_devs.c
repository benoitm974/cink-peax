...
#if defined(CONFIG_TINNO_FB)
#define RESERVED_MEM_SIZE_FOR_FB_TINNO (0x200000) //reserved 2MB
#else
#define RESERVED_MEM_SIZE_FOR_FB_TINNO (0x000000) //reserved 0MB
#endif
/*
...
#define TOTAL_RESERVED_MEM_SIZE (RESERVED_MEM_SIZE_FOR_PMEM + \
                                 ...
                                 RESERVED_MEM_SIZE_FOR_FB_TINNO)
...
#define FB_TINNO_START       (FB_START + FB_SIZE)
#define FB_TINNO_SIZE        (RESERVED_MEM_SIZE_FOR_FB_TINNO)
...
#if defined(CONFIG_TINNO_FB)
static u64 tinno_fb_dmamask = ~(u32)0;

static struct resource resource_fb_tinno[] = {
	{
		.start		= 0, /* Will be redefined later */
		.end		= 0,
		.flags		= IORESOURCE_MEM
...
static struct platform_device mt6577_device_fb_tinno = {
    .name = "tinno_fb",
    .id   = 1,
    .num_resources = ARRAY_SIZE(resource_fb_tinno),
    .resource      = resource_fb_tinno,
    .dev = {
        .dma_mask = &tinno_fb_dmamask,
        .coherent_dma_mask = 0xffffffff,
    },
};
#endif
...
__init int mt6577_board_init(void)
{
...

	{
		...
		key = 0;
#endif
	#if defined(TINNO_ANDROID_S9091_USB)
		memcpy(serial_number, "S9091", 5);
	#else
		if (key != 0)
...
"\n",   bl_fb.base, bl_fb.size, FB_START, FB_SIZE);

        {
            ...
            while (delay_sec >= 0) {
                printk("\rcontinue after %d seconds <Jieve>...", delay_sec--);
                mdelay(1000);
            }
            printk("\n");
        }
...
#if defined(CONFIG_TINNO_FB)
    resource_fb_tinno[0].start = FB_START;
    resource_fb_tinno[0].end   = FB_START + FB_SIZE - 1;

    printk("TINNO FB start: 0x%x end: 0x%x\n", resource_fb_tinno[0].start,
                                         resource_fb_tinno[0].end);

    retval = platform_device_register(&mt6577_device_fb_tinno);
    if (retval != 0) {
         return retval;
    }
#endif
