...
#if defined(TINNO_ANDROID_S9050_USB) || defined(TINNO_ANDROID_S9091_USB)
#define MANUFACTURER_STRING "Tinno"
#define PRODUCT_STRING "S9091 Android Phone"
#else
...
#if defined(TINNO_ANDROID_S9050_USB) || defined(TINNO_ANDROID_S9091_USB)
extern int get_host_status(void);
#endif
...
static void android_enable(struct android_dev *dev)
{
	...
	if (--dev->disable_depth == 0) {
		...
		if(!get_host_status()) {
			...
			mt6575_usb_info("%s: host mode, do not connect gadget--Liu\n", __func__);
		}	
	#else
		usb_gadget_connect(cdev->gadget);
	#endif
...
static void android_disable(struct android_dev *dev)
{
	...
	if (dev->disable_depth++ == 0) {
		...
		if(!get_host_status()) {
			...
			mt6575_usb_info("%s: host mode, do not disconnect gadget--Liu\n", __func__);
		}
	#else
		usb_gadget_disconnect(cdev->gadget);
	#endif
					...
					struct usb_composite_dev *cdev)
{
	...
	for(i = 0; i < config->fsg.nluns; i++) {
		...
		if(i == (config->fsg.nluns - 1)) {
			...
			config->fsg.luns[i].filename = CDROM_FILE;
		}		
	}
			    ...
			    const char *buff, size_t size)
{
	...
	mt6575_usb_info("%s: hostmode=%d, enabled=%d, dev->enabled=%d--Liu\n", 
		__func__, get_host_status(), enabled, dev->enabled);
...
#if defined(TINNO_ANDROID_S9050_USB) || defined(TINNO_ANDROID_S9091_USB)
static struct android_product usb_products[] = {
	{0x0BB4,		0, 			"none"},
	{0x0BB4,		0x2008, 		"mtp"},
	{0x0BB4,		0x0006, 		"acm"},
	{0x0BB4,		0x0C01, 		"adb"},
	{0x0BB4,		0x200E, 		"adb,acm"},
	{0x0BB4,		0x0001, 		"mass_storage"},
	{0x0BB4,		0x0C03, 		"mass_storage,adb"},
	{0x0BB4,		0x2012, 		"mtp,acm"},
	{0x0BB4,		0x0C02, 		"mtp,adb"},
	{0x0BB4,		0x200A, 		"mtp,adb,acm"},
	{0x0BB4,		0x2016, 		"mtp,mass_storage"},
	{0x0BB4,		0x2017, 		"mtp,mass_storage,adb"},
	{0x0BB4,		0x200B, 		"ptp"},
	{0x0BB4,		0x2013, 		"ptp,acm"},
	{0x0BB4,		0x200C, 		"ptp,adb"},
	{0x0BB4,		0x200D, 		"ptp,adb,acm"},
	{0x0BB4,		0x2015, 		"ptp,mass_storage"},
	{0x0BB4,		0x2014, 		"ptp,mass_storage,adb"},
	{0x0BB4,		0x0003, 		"rndis"},
	{0x0BB4,		0x2011, 		"rndis,acm"},
	{0x0BB4,		0x0004, 		"rndis,adb"},
	{0x0BB4,		0x2010, 		"rndis,adb,acm"},
	{0x0BB4,		0x0005, 		"mass_storage,adb,acm"},
	{0x0BB4,		0x2018, 		"mtp,mass_storage,acm"},
	{0x0BB4,		0x2019, 		"mtp,mass_storage,adb,acm"},
	{0x0BB4,		0x201A, 		"ptp,mass_storage,acm"},
	{0x0BB4,		0x201B, 		"ptp,mass_storage,adb,acm"},
	{0x18D1,		0x2D01, 		"accessory,adb"},
	{0x18D1,		0x2D00, 		"accessory"}
};

#define PRODUCT_SIZE (sizeof(usb_products) / sizeof(usb_products[0]))

static int product_has_acm(struct android_product product)
{
	int ret = 0;
	if(product.fuctions){
		if(strstr(product.fuctions, "acm")) {
			mt6575_usb_info("%s: fuctions=%s, has acm --Liu\n", 
				__func__, product.fuctions);
			ret = 1;
		}
		else {
			mt6575_usb_info("%s: fuctions=%s, no acm --Liu\n", 
				__func__, product.fuctions);
			ret = 0;
		}
	}
...
static int product_has_rndis(struct android_product product)
{
	int ret = 0;
	if(product.fuctions){
		if(strstr(product.fuctions, "rndis")) {
			mt6575_usb_info("%s: fuctions=%s, has rndis --Liu\n", 
				__func__, product.fuctions);
			ret = 1;
		}
		else {
			mt6575_usb_info("%s: fuctions=%s, no rndis --Liu\n", 
				__func__, product.fuctions);
			ret = 0;
		}
	}	
...
static struct android_product *get_product(uint32_t vid, uint32_t pid)
{
	int i = 0;
	for(; i < PRODUCT_SIZE; i++) {
		if((usb_products[i].vid == vid) && (usb_products[i].pid == pid))
			return &usb_products[i];
	}
	return &usb_products[0];
}

static ssize_t functions_ext_show(struct device *pdev, 
	struct device_attribute *attr, char *buf)
{
	struct android_dev *dev = dev_get_drvdata(pdev);
	struct android_usb_function *f;
	...
	list_for_each_entry(f, &dev->enabled_functions, enabled_list)
		buff += sprintf(buff, "%s,", f->name);
	mutex_unlock(&dev->mutex);
	mt6575_usb_info("%s: buf=%s--Liu\n", __func__, buf);
	if (buff != buf)
		*(buff-1) = '\n';
	return buff - buf;
}

static ssize_t functions_ext_store(struct device *pdev, 
		struct device_attribute *attr, const char *buff, size_t size)
{
	...
	INIT_LIST_HEAD(&dev->enabled_functions);
	mt6575_usb_info("%s: buff=%s--Liu\n", __func__, buff);
	strncpy(buf, buff, sizeof(buf));
	b = strim(buf);
	while (b) {
		name = strsep(&b, ",");
		mt6575_usb_info("%s: name=%s--Liu\n", __func__, name);
		if (name) {
			err = android_enable_function(dev, name);
			if (err)
				pr_err("android_usb: Cannot enable '%s' \n", name);
...
static ssize_t product_show(struct device *pdev, 
	struct device_attribute *attr, char *buf)
{
	struct android_dev *dev = dev_get_drvdata(pdev);
	struct android_usb_function *f;
...
static ssize_t product_store(struct device *pdev, 
		struct device_attribute *attr, const char *buff, size_t size)
{
	struct android_dev *dev = dev_get_drvdata(pdev);
	struct usb_composite_dev *cdev = dev->cdev;
	struct android_product *product = NULL;
	struct acm_function_config *config = NULL;
	unsigned long vid, pid;
	int err, buf_size;
	char buf[256], tmp[50], *b, *name; 
	char *start = buf;
	...
	while (*start == ' ')
		start++;
	vid = simple_strtoul(start, &start, 16);
	while (*start == ' ')
		start++;
	if (strict_strtoul(start, 16, &pid))
		return -EINVAL;
	product = get_product(vid, pid);

	//Disable functions and disconnect usb
	mutex_lock(&dev->mutex);	
	usb_gadget_disconnect(cdev->gadget);
	usb_ep_dequeue(cdev->gadget->ep0, cdev->req);
	usb_remove_config(cdev, &android_config_driver);
	...
	INIT_LIST_HEAD(&dev->enabled_functions);
	mt6575_usb_info("%s: buff=%s, fuctions=%s--Liu\n", __func__, buff, product->fuctions);
	strcpy(tmp, product->fuctions);
	b = strim(tmp);
	while (b) {
		name = strsep(&b, ",");
		mt6575_usb_info("%s: name=%s--Liu\n", __func__, name);
	...
	//Do specific for acm
	if(product_has_acm(*product)) {
		if(acm_function.config) {
			config = acm_function.config;
			config->instances = 1;
		}
	}
	if(product_has_rndis(*product)) {
		
	}
	
	//Add new functions and connect usb
	cdev->desc.idVendor = (uint16_t)vid;
	cdev->desc.idProduct = (uint16_t)pid;
	cdev->desc.bcdDevice = device_desc.bcdDevice;
	cdev->desc.bDeviceClass = device_desc.bDeviceClass;
	cdev->desc.bDeviceSubClass = device_desc.bDeviceSubClass;
	cdev->desc.bDeviceProtocol = device_desc.bDeviceProtocol;
...
field ## _show(struct device *dev, struct device_attribute *attr,	\
		char *buf)						\
{									\
	mt6575_usb_info("%s: format_string=%s--Liu\n", __func__, format_string);	\
...
field ## _show(struct device *dev, struct device_attribute *attr,	\
		char *buf)						\
{									\
	mt6575_usb_info("%s: buffer=%s--Liu\n", __func__, buffer);	\
...
#if defined(TINNO_ANDROID_S9050_USB) || defined(TINNO_ANDROID_S9091_USB)
static DEVICE_ATTR(functions_ext, S_IRUGO | S_IWUSR, functions_ext_show, functions_ext_store);
static DEVICE_ATTR(product, S_IRUGO | S_IWUSR, product_show, product_store);
#endif
...
#if defined(TINNO_ANDROID_S9050_USB) || defined(TINNO_ANDROID_S9091_USB)
	strncpy(serial_string, "S9091", sizeof(serial_string) - 1);
#else
...
#ifdef USB_CDROM
	INIT_DELAYED_WORK(&cdrom_work, do_cdrom_work);
#endif
