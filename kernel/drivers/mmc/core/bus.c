...
static void mmc_decode_sepc(unsigned char spec, unsigned char spec3, char *buf)
{
	if(!spec)
		sprintf(buf, "Version 1.0 or 1.01");
	else if(spec == 1) 
		sprintf(buf, "Version 1.10");
	else if(spec == 2) {
		if(!spec3)
			sprintf(buf, "Version 2.0");
		else
			sprintf(buf, "Version 3.0");		
	}
}
static void mmc_decode_class(unsigned char speed_class, char *buf) 
{
	int class_id = 0;
	switch(speed_class) {
		case 0:
	...
	sprintf(buf, "class %x", class_id);
}
static void mmc_decode_security(unsigned char security, char *buf)
{
	if(!security)
		sprintf(buf, "No Security");
	else if(security == 1)
		sprintf(buf, "Not Used");
	else if(security == 2)
		sprintf(buf, "Security Version 1.01");
	else if(security == 3)
		sprintf(buf, "Security Version 2.0");
	else if(security == 4)
		sprintf(buf, "Security Version 3.xx");
}
#endif
/*
